...
val_size = len(cifar) - train_size
train_cifar, val_cifar = random_split(cifar, [train_size, val_size])

# Dataloaders
batch_size = 64
train_loader = DataLoader(train_cifar, batch_size=batch_size, shuffle=True)
val_loader = DataLoader(val_cifar, batch_size=batch_size, shuffle=False)
test_loader = DataLoader(test_cifar, batch_size=batch_size, shuffle=False)

# Model parameters
input_size = 32 * 32 * 3  # CIFAR10 input size
hidden_size = 128
num_classes = 10
epochs = 20
